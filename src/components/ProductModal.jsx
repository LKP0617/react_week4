...
function ProductModal({ modalMode, selectedProduct, isOpen, setIsOpen, getProducts }) {
    ...
    const createProduct = async () => {
        try {
            await axios.post(`${BASE_URL}/v2/api/${API_PATH}/admin/product`, { data: { ...modalData, origin_price: Number(modalData.origin_price), price: Number(modalData.price), is_enabled: modalData.is_enabled ? 1 : 0 } });
            getProducts();
            setIsContent(true);
        } catch (error) {
            alert('新增產品失敗');
    ...
    const updateProduct = async () => {
        try {
            await axios.put(`${BASE_URL}/v2/api/${API_PATH}/admin/product/${modalData.id}`, { data: { ...modalData, origin_price: Number(modalData.origin_price), price: Number(modalData.price), is_enabled: modalData.is_enabled ? 1 : 0 } });
            getProducts();
            setIsContent(true);
        } catch (error) {
            alert('編輯產品失敗');
    ...
    const handleUpdateProduct = async () => {
        if (!modalData.title || !modalData.category || !modalData.unit || !modalData.origin_price) {
            alert("請填寫完整產品資訊：標題、分類、單位和價格皆為必填項目");
            if (modalData.price === 0) {
                modalData.price = modalData.origin_price;
            }
            return;
        }
    ...
    const handleFileChange = async (e) => {
        ...
        try {
            ...
            const uploadedImgUrl = res.data.imageUrl;
